    ...
    static let mechanics = KnowledgeModule(
        ...
        chapters: [
            KnowledgeChapter(id: "kinematics", name: "运动的描述与直线运动", icon: "ruler", points: [
                KnowledgePoint("k_basic", "质点·参考系·位移", essence: "研究运动先选参考系；位移是矢量、路程是标量。", formula: "Δx = x₂ − x₁", stage: .junior),
                KnowledgePoint("k_va", "速度·加速度", essence: "速度是位置变化率，加速度是速度变化率——和速度方向无必然关系。", formula: "v = Δx/Δt，a = Δv/Δt"),
                KnowledgePoint("k_uniform", "匀变速直线运动", essence: "三大公式串起 v、x、t、a；缺谁选谁。", formula: "v=v₀+at；x=v₀t+½at²；v²=v₀²+2ax", pitfall: "矢量正负方向先统一，再代数运算。"),
                KnowledgePoint("k_graph", "v-t / x-t 图象", essence: "斜率是变化率，面积是累积量——图象法秒杀运动学。", weapon: .graphMethod, pitfall: "别把 x-t 图的斜率当加速度。"),
            ]),
            KnowledgeChapter(id: "force", name: "相互作用·力", icon: "arrow.up.and.down.and.arrow.left.and.right", points: [
                KnowledgePoint("f_gravity", "重力·弹力·摩擦力", essence: "三种常见力的来源与方向；弹力垂直接触面，摩擦力沿接触面。", formula: "G=mg；F弹=kx；f=μN", stage: .junior),
                KnowledgePoint("f_compose", "力的合成与分解", essence: "平行四边形定则；按效果或正交分解。", formula: "F=√(F₁²+F₂²+2F₁F₂cosθ)"),
                KnowledgePoint("f_diagram", "受力分析", essence: "隔离 → 画力 → 正交分解，三步不漏力。", weapon: .forceDiagram, pitfall: "凭空捏造「运动方向的力」是最大坑。"),
            ]),
    ...
    static let electromagnetism = KnowledgeModule(
        ...
        chapters: [
            ...
            KnowledgeChapter(id: "ac", name: "交变电流", icon: "waveform.path.ecg", points: [
                ...
                KnowledgePoint("ac_transformer", "变压器·输电", essence: "理想变压器电压比等于匝数比。", formula: "U₁/U₂=n₁/n₂"),
            ]),
        ])
    ...
    static let thermal = KnowledgeModule(
        ...
        chapters: [
            ...
            KnowledgeChapter(id: "thermo", name: "热力学定律", icon: "flame", points: [
                ...
                KnowledgePoint("th_second", "热力学第二定律", essence: "热自发地从高温流向低温，宏观过程有方向性。", pitfall: "并非否定能量守恒，而是约束过程方向。"),
            ]),
        ])
    ...
    static let modern = KnowledgeModule(
        ...
        chapters: [
            ...
            KnowledgeChapter(id: "nucleus", name: "原子核", icon: "burst", points: [
                ...
                KnowledgePoint("nu_mass", "核反应·质能方程", essence: "质量亏损释放巨大能量；裂变聚变之源。", formula: "E=mc²", weapon: .dimensionalAnalysis),
            ]),
        ])
}

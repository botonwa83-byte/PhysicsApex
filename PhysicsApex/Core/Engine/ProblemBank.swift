...
enum ProblemBank {
    ...
    static let all: [PhysicsProblem] = {
        ...
        for batch in [batch1, batch2, batch3, batch4, batch5, batch6, batch7, batch8, batch9] {
            result += batch
        }
        return result
    }()
    ...
    static let collisionDescent = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "想象两团橡皮泥相撞后粘在一起：撞的瞬间里面怎么挤、怎么变形都不用管——因为外面没有人推它们，「总冲劲」（质量×速度加在一起）撞前撞后必须一样多。所以用撞前的总冲劲除以总质量，就是粘在一起后的速度。少掉的动能去哪了？全变成了热。"
        ),
        tags: ["碰撞", "动量守恒", "降维"]
    )
    ...
    static let riverCrossingDescent = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "把船速想成一根能随意转方向的「指针」：水速是固定的一支箭，船速指针绕着箭头尖画一个圆。你真正的前进方向，就是从起点连到圆上某一点。想怎么走最划算？在圆上挑那个最合适的点就行——图一画答案就在眼前，根本不用求导。"
        ),
    ...
    static let magneticPeriodChoice = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "转一圈的时间＝一圈的路程÷速度。粒子跑得快一倍，它画的圆也恰好大一倍——跑得快但路也变远了，两件事正好抵消。所以转一圈的时间跟快慢根本没关系，只看粒子本身（m、q）和磁场（B）。下次再问「速度变了周期变不变」，闭着眼选不变。"
        ),
        misconceptions: [
            Misconception(
                option: "周期变为原来的 2 倍",
    ...
    static let railTerminalVelocity = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "棒越滑越快 → 感应电流越来越大 → 拖它的安培力也越来越大 → 加速越来越费劲……总有一刻，拖力恰好追平推力，速度就此封顶。我们不去管中间那段越来越慢的纠缠，直接跳到「封顶」那一刻：受力平衡，列一个方程，最大速度立刻出来。"
        ),
        tags: ["电磁感应", "终速度", "平衡", "降维"]
    )
    ...
    static let workEnergyMultiStage = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "别盯着那段弯弯曲曲的轨道发愁。动能定理就像月底查银行卡：只看期初余额（初动能）、期末余额（末动能）、这个月总共进出多少钱（各力做的功），三个数必须对得上账。中间你去过哪些商店（路径多复杂）银行根本不关心。"
        ),
        tags: ["动能定理", "多过程", "降维"]
    )
    ...
    static let nearEarthSatellite = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "不知道地球质量？没关系——地面上每一个下落的苹果都已经替你「测」过了：苹果受的重力 mg 就是万有引力 GMm/R²，约掉 m 得到 GM=gR²。把查不到的 GM 换成天天见的 g 和 R，卫星速度一步就出来。这招叫黄金代换，天体题十有八九用得上。"
        ),
    ...
    static let electricDeflection = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "电子在电场里的偏转，其实就是「横过来的平抛」：沿板方向没有力管它——匀速直线；垂直板方向有恒定电场力拽它——匀加速。把平抛运动的所有公式原封不动搬过来用就行，连「飞行时间只由水平长度决定」这种老结论都能直接套。"
        ),
        tags: ["电场", "类平抛", "等效迁移", "降维"]
    )
    ...
    static let photoEffectChoice = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "光电效应是「一个光子赎一个电子」的买卖：光子揣着能量 hν 来，先付一笔「赎身费」W₀ 把电子从金属里赎出来，剩下的钱全给电子当动能。光再强也只是光子「人多」，单个光子钱不够，谁也赎不出来——这就是为什么微弱的紫光行、刺眼的红光不行。"
        ),
        misconceptions: [
            Misconception(
                option: "增大光照强度",
    ...
    static let doubleSlitDescent = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "条纹间距公式 Δy=Lλ/d 就是一把「光的尺子」，三个旋钮摆在那：波长 λ 越长条纹越宽、屏越远（L 大）条纹越宽、双缝越近（d 小）条纹越宽。题目只动了哪个旋钮？盯住它按比例放大缩小就行，别的数字一个都不用碰。"
        ),
    ...
    static let waterJetForce = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "整条水柱撞墙没法下手？那就切片！只盯着「这 1 秒内撞上去的那一小段水」：它有多重（流量×1秒）、速度从 v 直接归零，冲劲的变化除以这 1 秒，就是墙挨的力。连续不断的难题，切成一小片就变成最普通的小学算术。"
        ),
        tags: ["微元法", "动量", "流体冲力", "竞赛"]
    )
    ...
    static let triangleCharge = PhysicsProblem(
        ...
        dualSolution: DualSolution(
            ...
            plainTalk: "动笔前先看「长相」：两个电荷对称地摆在左右，那它们对中间那个电荷的推力，横向部分必然一边一半互相抵消——这是图形对称白送的结论，连算都不用算。剩下的只有沿对称轴方向做一次加法。对称看出来了，一半的计算直接蒸发。"
        ),

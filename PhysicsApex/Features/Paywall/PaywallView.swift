...
struct PaywallView: View {
    ...
    private var priceLabel: String { purchase.product?.displayPrice ?? "¥18" }
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 0) {
                ...
                VStack(spacing: 6) {
                    Text("免费已开放 \(PurchaseManager.freeDescentCount) 道战例 + \(PurchaseManager.freeRevisitCount) 个重访")
